...
- Put case-centered speaking records in `Speaking_Cases/`.
...
- Keep article links, brief excerpts, and user summaries in speaking cases; do not put full copyright-uncertain articles in this public repository.
...
- A `speaking_case` inbox update creates a Daily Session, a Speaking Case, and linked knowledge nodes.
- A `review_update` inbox update changes only the target case review metadata and writes one history entry.
- Case nodes are the review units. Vocabulary, grammar, expressions, responses, and stories remain browseable support material.
- Use `prompts/chatgpt_project_instructions.md` as the durable ChatGPT Project instruction source. GPT should only save after the user says `保存案例`.
